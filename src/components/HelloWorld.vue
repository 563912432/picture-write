    ...
    <div class="content">
      <ul v-loading.body="loading">
        <li v-for="(item, index) in msg" :key="item.id"
            ...
            :title="item.title"
            @click="click(item.id)">
          <i class="el-icon-arrow-right" :class="[index % 2 === 0?'i-blue':'i-white']" style="font-weight: bold;"></i>
...
export default {
  ...
  mounted () {
    this.loading = true
    // 取cookie
    this.openSimple = !this.$cookie.get('fpw')
    let url = Host + 'pictureWriting'
    this.post(url, '', res => {
      this.loading = false
      if (res.status) {
        this.info = res.info
  ...
  methods: {
    ...
    bClose () {
      ...
      this.$cookie.set('fpw', 'Helloworld', parseInt(this.info.adInfo.interval))
    }
  }
}
</script>
